...
- **不含播放器**：解出交错 Float32 PCM 后，AVAudioEngine 播放逻辑由使用方自行实现。

## 结构

```
Scripts/build-ffmpeg.sh            从源码 audio-only 交叉编译 → xcframework
Artifacts/FFmpegAudio.xcframework  构建产物（纯 ffmpeg 静态库，需先跑脚本生成）
Sources/CFFmpegAudio/              C wrapper：decode / seek / probe / metadata
Sources/FFmpegAudioKit/            通用 Swift API（零业务依赖）
    FFmpegAudioDecoder             解码：PCM → AVAudioPCMBuffer（交错 Float32）
...
3. 合并静态库并 `xcodebuild -create-xcframework`，产出 `ios-arm64` + `ios-arm64_x86_64-simulator`。
...
while let buffer = try decoder.nextBuffer() {
    // buffer 为交错 Float32 的 AVAudioPCMBuffer
}
